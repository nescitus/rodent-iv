...
int cEngine::ScaleKNPK(POS *p, eColor sd, eColor op) {
    ...
    &&  p->mCnt[op][P] == 0) {
        ...
        if ((RelSqBb(A7, sd) & p->Map(sd, P))
        && (RelSqBb(A8, sd) & p->Map(op, K))) return 0; // dead draw

        if ((RelSqBb(H7, sd) & p->Map(sd, P))
        && (RelSqBb(H8, sd) & p->Map(op, K))) return 0; // dead draw
    }
