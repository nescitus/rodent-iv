...
void sBook::OpenPolyglot() {
    ...
    if (!isabsolute(bookName))                  // if known locations don't exist we want to load only from absolute paths
        if (!ChDirEnv("RODENT4BOOKS"))          // try `RODENT4BOOKS` env var first (26/08/17: linux only)
...
int sBook::GetPolyglotMove(POS *p, bool print_output) {
    ...
    for (int i = 0; i < n_of_choices; i++) {
        ...
        if (values[i] > -1 || max_weight == 1) {
            if (print_output && Glob.isNoisy) {
                printfUciOut("info string ");
                PrintMove(moves[i]);
                if (IsInfrequent(values[i], max_weight)) printfUciAdd("?!\n");
        ...
        if (!IsInfrequent(values[i], max_weight)) {
            vals_acc += values[i];
            if (PolyglotRandom(vals_acc) < values[i]) best_move = moves[i];
        }
    }
...
bool sBook::IsInfrequent(int val, int max_freq) {
    ...
    if (val < ((max_freq * Par.bookFilter) / 100)) return true; // rare moves get filtered out
    return false;
}
